...
4. Do not assume the host app already loaded profile, history, or media for you. If you need context, call a script.
5. Treat `text + media` in the same user turn as one unit: the text is the question, the media is the evidence.
6. If the user says "this/this photo/this video" and the target is ambiguous, first read context and list recent media.
...
Every script call is scoped to the current user via environment variables injected by the host application:
...
Use when the user says something like "what were we talking about", "continue", "what about this", or refers to an earlier turn.
...
### 2. User sends photo/video and asks "check this out"

1. Read the user text in the same turn
2. `bash scripts/list-recent-media.sh --active-only`
3. `bash scripts/inspect-media.sh --media-id ... --question "<user's exact words>" --domain ...`
4. Answer from the script result

### 3. Later follow-up like "how much weight is this / did you see it"

1. `bash scripts/get-context.sh --scope recent`
2. `bash scripts/list-recent-media.sh --active-only`
3. Pick the correct `media_id`
4. `bash scripts/inspect-media.sh --media-id ... --question "<user's exact words>" --domain training`
...
bash scripts/inspect-media.sh --media-id med_20260301_140100_a0 --question "How much weight is this power clean?" --domain training
